...
public class dbDocumentoToce {
    ...
    public dbDocumentoToce() {
    }

    public boolean dbRicalcolaProgressivo(String stato, String data, JTextComponent texNumePrev, JTextComponent texAnno) {
        ...
        if (stato.equals(frmTestDocu.DB_INSERIMENTO)) {
            ...
            try {
                ...
                if (resu.next() == true) {
                    ...
                    if (texNumePrev.getText().length() == 0 || !texNumePrev.getText().equalsIgnoreCase(resu.getString(1))) {
    ...
    public String convertiInFattura(boolean raggr, boolean raggr_riepilogo, boolean use_serie) {
        ...
        newSerie = Db.nz(this.useSerie ? this.serie : "", "");
        newStato = "P";
...
//            String daddt = "Da DDT ";
            ...
            try {
                ...
                Logger.getLogger(dbDocumentoToce.class.getName()).log(Level.SEVERE, null, ex);
            }
            System.out.println("daddt:" + daddt);
            sql = "select r.*, t.data as dataddt, sum(quantita) as sqta from " + test_ddt + " t ";
            ...
            if (raggr_riepilogo) {
                ...
                for (int i2 = 0; i2 < this.elencoDdt.size(); i2++) {
                    ...
                    sql += " order by t.serie, t.anno, t.numero, r.riga";
                    tempPrev = Db.openResultSet(sql);
                    inserisciRighe(raggr, i2, null, tempPrev, newNumero, newStato, newSerie);
                }
            }
        }
        ...
        if (!raggr && !raggr_riepilogo) {
            ...
            JDialogSceltaQuantita dialog = new JDialogSceltaQuantita(main.getPadre(), true);
...
//        }
        ...
        if (acquisto) {
            ...
            sql += " , iva = " + Db.pc(tempFattura.totaleIva, "DOBULE");
            sql += " , importo = " + Db.pc(tempFattura.totale, "DOBULE");
        } else {
            ...
            sql += " , totale_iva = " + Db.pc(tempFattura.totaleIva, "DOBULE");
            sql += " , totale = " + Db.pc(tempFattura.totale, "DOBULE");
        }
    ...
    public boolean generaMovimentiMagazzino() {
        ...
        sql = "delete from movimenti_magazzino";
        ...
        while (righe.next()) {
            ...
            if (inseribile == true) {
                ...
                if (cacheArticoli != null && cacheArticoli.containsKey(codart)) {
                    ...
                    sql = "select flag_kit from articoli where codice = '" + Db.nz(righe.getString("codice_articolo"), "") + "'";
                ...
                if (!kit) {
                    ...
                    if (tabella.equals("pacchetti_articoli")) {
                        ...
                        sql += "articolo = '" + Db.nz(righe.getString("codice_articolo"), "") + "' and pacchetto = '" + pacchetto + "'";
                    ...
                    sql += " where pacchetto = '" + Db.nz(righe.getString("codice_articolo"), "") + "'";
...
//            SwingUtils.showErrorMessage(null, "Errore tabella ddt acquisto MATRICOLE");
    ...
    private void inserisciRigheRiepilogative(int i2, String daddt, ResultSet tempPrev, int newNumero, String newStato, String newSerie) {
        ...
        try {
            ...
            sql = "select serie,numero,data,anno from " + test_ddt + " t ";
            sql += elencoDdt.get(i2);
            sql += " order by serie, anno, numero";
            System.out.println("DDT_FATT_SQL>" + sql);
            tempDdtProv = Db.openResultSet(sql);
            ...
            doc.load(Db.INSTANCE, tempDdtProv.getInt("numero"), tempDdtProv.getString("serie"), tempDdtProv.getInt("anno"), tipodoc);
            doc.calcolaTotali();
            doc.visualizzaCastellettoIva();
...
//
//                for (int i = 1; i <= metaPrev.getColumnCount(); i++) {
//                    if (metaPrev.getColumnName(i).equalsIgnoreCase("id")) continue;
//                    if (metaPrev.getColumnName(i).equalsIgnoreCase("data")) continue;
//                    if (metaPrev.getColumnName(i).equalsIgnoreCase("dataddt")) continue;
//                    if (metaPrev.getColumnName(i).equalsIgnoreCase("sqta")) continue;
//
//                    if (metaPrev.getColumnName(i).equalsIgnoreCase("numero")) {
//                        sqlC += "numero";
//                        sqlV += Db.pc(String.valueOf(newNumero), metaPrev.getColumnType(i));
...
//                    }
//                }
//                if (sqlC.endsWith(",")) sqlC = sqlC.substring(0, sqlC.length()-1);
//                if (sqlV.endsWith(",")) sqlV = sqlV.substring(0, sqlV.length()-1);
//
//                sql = "insert into righ_fatt ";
//                sql += "(" + sqlC + ") values (" + sqlV + ")";
//
//                //debug
//                //System.out.println("sql righe:" + sql);
//                Db.executeSql(sql);
//            }
